...
class Orbit:
    ...
    def vector_function_right_parts(self, t: float, y: np.ndarray):
        ...
        R = 6378 * 10 ** 3
        # Коэффициент J2, характеризующий асимметрию массы Земли
        J2 = 1082.8 * 10 ** -6

        WJ2 = 3/2 * J2 * self.mu * R ** 2 / (np.linalg.norm(y[0:3])) ** 7 * np.array([
            -y[0] * (y[0] ** 2 + y[1] ** 2 - 4 * y[2] ** 2),
            -y[1] * (y[0] ** 2 + y[1] ** 2 - 4 * y[2] ** 2),
            -3 * y[2] * (y[0] ** 2 + y[1] ** 2) + 2 * y[2] ** 3
        ])
        right_parts = np.zeros(6)
        right_parts[0:3] = y[3:6]
        right_parts[3:6] = - self.mu * y[0:3] / (np.linalg.norm(y[0:3])) ** 3 + WJ2
        return right_parts
    ...
    def calculate_first_integral(self, y: np.ndarray):
        """
        ...
        # Коэффициент J2, характеризующий асимметрию массы Земли
        J2 = 1082.8 * 10 ** -6
        P2 = -3/2 * J2 * self.mu * R ** 2 / (np.linalg.norm(r, axis=1) ** 3) * \
            (1/3 - z ** 2 / (np.linalg.norm(r, axis=1) ** 2))
        self.result_E = (np.linalg.norm(v, axis=1) ** 2) / 2 - self.mu / np.linalg.norm(r, axis=1) + P2
    ...
    def add_results(self, t: np.ndarray, y: np.ndarray):
        ...
        self.calculate_first_integral(y=y)

    def theoretical_calculations_of_orbit_change(self):
        """
        Эта функция выполняет теоретические расчеты изменения орбиты космического объекта, учитывая влияние
        гравитационных искажений Земли. Функция использует формулы, основанные на коэффициенте J2 и других параметрах,
        чтобы предсказать изменения орбиты космического объекта из-за гравитационных воздействий Земли.

           :return: delta_omega и delta_w.
        """
        # Радиус Земли в метрах
        R = 6378 * 10 ** 3
        # Коэффициент J2, характеризующий асимметрию массы Земли
        J2 = 1082.8 * 10 ** -6
        # Вычисление параметра орбиты (первого фокусного расстояния)
        p = self.a * (1 - self.e ** 2)

        # Изменение аргумента перицентра
        delta_w = np.pi * 3 / 2 * J2 * R ** 2 / p ** 2 * (5 * np.cos(self.i) ** 2 - 1)
        # Изменение долготы восходящего узла
        delta_omega = - np.pi * 3 * J2 * R ** 2 / p ** 2 * np.cos(self.i)

        return [delta_omega, delta_w]
